...
int main(int argc, char** argv) {
    ...
    for (;;) {
        ...
        while (not_done) {
            ...
            if (packet.frag_no == packet.total_frag) {  // break out of the loop for the last fragment
                fclose(fps);
            ...
            char* respondMsg = "ACK";
            if (sendto(mySocketfd, respondMsg, strlen(respondMsg), 0, (struct sockaddr*)&from, fromLen) == -1) {
